...
namespace BlinkCash.Core.Utilities
{
    ...
    {
        public static string GenerateTnxReference(this Guid value, string prefix)
        {
            ShortGuid shortguid = value;
            return $"prefix{shortguid.ToString().Replace('-', '0').Replace('_', '0')}".ToUpper(); //Constants.TransactionRefCode + shortguid.ToString().Replace('-', '0').Replace('_', '0').ToUpper();
        }
